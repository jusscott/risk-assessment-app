...
MAX_RETRIES=30
COUNT=0
until pg_isready -h payment-db -U postgres || [ $COUNT -eq $MAX_RETRIES ]; do
    echo "Waiting for payment database to be ready... ($COUNT/$MAX_RETRIES)"
    sleep 2
    COUNT=$((COUNT+1))
done
...
echo "Seeding the database with initial plans..."
node prisma/seed.js
