...
const createServiceProxy = (options) => {
  ...
  const proxy = createProxyMiddleware({
    ...
    onProxyReq: (proxyReq, req, res) => {
      ...
      // CRITICAL: Always forward Authorization header to maintain session state across services
      if (req.headers.authorization) {
        ...
        logger.debug(`Forwarding authorization token ${tokenPrefix} to ${serviceName}`, { 
          requestId, 
          service: serviceName 
        });
      } else {
        logger.debug(`No authorization token to forward to ${serviceName}`, { 
          requestId, 
          service: serviceName 
        });
      }
